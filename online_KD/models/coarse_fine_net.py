...
class spaced_distillation (nn.Module):
    def __init__(self,net):
        ...
        self.linear1 = nn.Linear(self.student.feature_size, self.student.feature_size)
        self.linear2 = nn.Linear(self.student.feature_size, self.teacher.feature_size)
        self.relu = nn.ReLU()

    def forward(self,x):
        feature_teacher, pred_teacher =self.teacher(x)

        feature_student, pred_student =self.student(x)

        feature_student = self.linear1(feature_student)
        feature_student = self.relu(feature_student)
        feature_student = self.linear2(feature_student)
        return feature_teacher, pred_teacher, feature_student, pred_student
